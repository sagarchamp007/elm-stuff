...
-- VIEW PARSER
parse_view string string1 parse_res grid = let 
                       input_view = div []
                                   [stringInput string
                                    ,drawButton string
                                   ]
                       ...
                       grid_d =  div [myGridStyle] [Html.fromElement <| grid]
                                           in div [mydivStyle] [input_view,parse_out,grid_d]
...
-- VIEW GRID
view_grid conf_data = div[][]

main =
       let
          a = Signal.map drawGrid grid_mail.signal
       in Signal.map4 (parse_view) prog_input.signal result.signal parse_out.signal a
...
grid_mail =
  Signal.mailbox emptyGrid  


port requests: Signal (Task x ())
port requests =  Signal.map parse_it result.signal |> Signal.map (\res -> Signal.send parse_out.address res)
            
-- init_status : Signal (Task x ())
-- init_status =
--                let
--                    initialGrid = Signal.map (init_grid) config.signal
--                in Signal.map2 getInitStatus initialGrid parse_out.signal |> Signal.map (\state -> Signal.send newStatus.address state)
...
port new_status =
            let
              ...
              pressed = presses
            in Signal.map5 getStatus pressed newStatus.signal initInfo initialGrid parse_out.signal|> Signal.map (\state -> Signal.send newStatus.address state)

port extract_grid : Signal (Task x ())
port extract_grid  =
                    let
                      initialGrid = Signal.map (init_grid) config.signal 
                    in Signal.map2 (getGrid) newStatus.signal initialGrid |> Signal.map (\gr-> Signal.send grid_mail.address gr)
